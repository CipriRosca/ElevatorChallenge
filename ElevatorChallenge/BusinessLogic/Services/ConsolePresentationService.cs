...
namespace ElevatorChallenge.BusinessLogic.Services
{
    public class ConsolePresentationService : IConsolePresentationService
    {
        ...
        {
            elevator.MovedToFloor += Elevator_MovedToFloor;
        }

        private void Elevator_MovedToFloor(Elevator sender, int floor)
        {
            DisplayMessage($"Elevator {sender.Id} is now on floor {floor}.");
        }

        private void Elevator_PassengersBoarded(Elevator sender, int numberOfPassengers)
        {
            DisplayMessage($"{numberOfPassengers} passengers boarded on Elevator {sender.Id}.");
        }
        ...
        {
            DisplayMessage(new string('-', 100));
        }

        public void DisplayHeader(string headerText)
        {
            DisplayMessage($"===== {headerText} =====");
        }
